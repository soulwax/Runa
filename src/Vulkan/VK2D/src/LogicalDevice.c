// File: src/Vulkan2D/VK2D/src/LogicalDevice.c
...
VK2DLogicalDevice vk2dLogicalDeviceCreate(VK2DPhysicalDevice dev,
  ...
  uint32_t queueFamily = graphicsDevice == true
                             ? dev->QueueFamily.graphicsFamily
                             : dev->QueueFamily.computeFamily;

  // Find any optional extensions
  uint32_t extensionCount = 0;
  VkExtensionProperties *props = VK_NULL_HANDLE;
  vkEnumerateDeviceExtensionProperties(dev->dev, VK_NULL_HANDLE,
                                       &extensionCount, VK_NULL_HANDLE);
  props = malloc(extensionCount * sizeof(struct VkExtensionProperties));
  vkEnumerateDeviceExtensionProperties(dev->dev, VK_NULL_HANDLE,
                                       &extensionCount, props);
  const bool instanceExtensionSupported = gRenderer->limits.supportsVRAMUsage;
  gRenderer->limits.supportsVRAMUsage = false;
  for (int i = 0; i < extensionCount; i++) {
    if (strcmp(props[i].extensionName, VK_EXT_MEMORY_BUDGET_EXTENSION_NAME) ==
            0 &&
        instanceExtensionSupported)
  ...
  if (ldev != NULL) {
    ...
    VkDeviceQueueCreateInfo queueCreateInfo =
        vk2dInitDeviceQueueCreateInfo(queueFamily, priority);
    queueCreateInfo.queueCount =
        gRenderer->limits.supportsMultiThreadLoading ? 2 : 1;
    VkDeviceQueueCreateInfo queues[] = {queueCreateInfo};
    VkDeviceCreateInfo deviceCreateInfo =
        vk2dInitDeviceCreateInfo(queues, 1, &feats, debug);
    ...
    // Create device
    VkResult result =
        vkCreateDevice(dev->dev, &deviceCreateInfo, VK_NULL_HANDLE, &ldev->dev);
    if (result != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create logical device, Vulkan error %i.", result);
      free(ldev);
      return NULL;
    }
    ldev->pd = dev;
    ...
    VkCommandPoolCreateInfo commandPoolCreateInfo =
        vk2dInitCommandPoolCreateInfo(
            queueFamily, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
    result = vkCreateCommandPool(ldev->dev, &commandPoolCreateInfo,
                                 VK_NULL_HANDLE, &ldev->pool);
    if (result != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create command pool, Vulkan error %i.", result);
      free(ldev);
      return NULL;
    }
    ...
    if (gRenderer->limits.supportsMultiThreadLoading) {
      ...
      ldev->workerThread =
          SDL_CreateThread(_vk2dWorkerThread, "VK2D_Load", NULL);

      if (ldev->loadListMutex == NULL || ldev->workerThread == NULL ||
          ldev->shaderMutex == NULL) {
        vk2dRaise(VK2D_STATUS_SDL_ERROR,
                  "Failed to initialize worker thread, SDL error: %s",
                  SDL_GetError());
...
void vk2dLogicalDeviceResetPool(VK2DLogicalDevice dev) {
  ...
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to reset command pool, Vulkan error %i", result);
  }
}
...
VkCommandBuffer vk2dLogicalDeviceGetCommandBuffer(VK2DLogicalDevice dev,
  ...
  VkCommandBufferAllocateInfo allocInfo =
      vk2dInitCommandBufferAllocateInfo(dev->pool, 1);
  ...
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to get command buffer, Vulkan error %i", result);
  }
  return buffer;
}
...
void vk2dLogicalDeviceGetCommandBuffers(VK2DLogicalDevice dev, bool primary,
  ...
  VkCommandBufferAllocateInfo allocInfo =
      vk2dInitCommandBufferAllocateInfo(dev->pool, n);
  allocInfo.level = primary ? VK_COMMAND_BUFFER_LEVEL_PRIMARY
                            : VK_COMMAND_BUFFER_LEVEL_SECONDARY;
...
VkCommandBuffer vk2dLogicalDeviceGetSingleUseBuffer(VK2DLogicalDevice dev,
  ...
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to allocate command buffers, Vulkan error %i", result);
  }
  ...
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to begin command buffer, Vulkan error %i", result);
  }
  return buffer;
}
...
void vk2dLogicalDeviceSubmitSingleBuffer(VK2DLogicalDevice dev,
  ...
  VkSubmitInfo submitInfo = vk2dInitSubmitInfo(
      &buffer, 1, VK_NULL_HANDLE, 0, VK_NULL_HANDLE, 0, VK_NULL_HANDLE);
  ...
  if (mainThread) {
    ...
    if (result == VK_SUCCESS) {
      ...
      if (result == VK_SUCCESS) {
        ...
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to wait for queue, Vulkan error %i", result);
      }
    } else {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
...
VkFence vk2dLogicalDeviceGetFence(VK2DLogicalDevice dev,
  ...
  VkFence fence;
  VkResult result =
      vkCreateFence(dev->dev, &fenceCreateInfo, VK_NULL_HANDLE, &fence);
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to create fence, Vulkan error %i", result);
  }
  return fence;
}
...
VkSemaphore vk2dLogicalDeviceGetSemaphore(VK2DLogicalDevice dev) {
  ...
  VkResult result = vkCreateSemaphore(dev->dev, &semaphoreCreateInfo,
                                      VK_NULL_HANDLE, &semaphore);
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to create semaphore, Vulkan error %i", result);
  }
  return semaphore;
}
