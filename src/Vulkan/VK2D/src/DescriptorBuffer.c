...
static _VK2DDescriptorBufferInternal *
_vk2dDescriptorBufferAppendBuffer(VK2DDescriptorBuffer db) {
  ...
  if (db->bufferCount == db->bufferListSize) {
    db->buffers = realloc(
        db->buffers, sizeof(_VK2DDescriptorBufferInternal) *
                         (db->bufferListSize + VK2D_DEFAULT_ARRAY_EXTENSION));
    db->memoryBarriers =
        realloc(db->memoryBarriers,
  ...
  buffer->size = 0;
  buffer->stageBuffer =
      vk2dBufferCreate(db->dev, db->pageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
...
void vk2dDescriptorBufferBeginFrame(VK2DDescriptorBuffer db,
  ...
  for (int i = 0; i < db->bufferCount; i++) {
    ...
    db->buffers[i].size = 0;
    VkResult result =
        vmaMapMemory(gRenderer->vma, db->buffers[i].stageBuffer->mem,
                     &db->buffers[i].hostData);
    if (result != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR, "Failed to map memory, VMA error %i.",
                result);
      return;
    }
  }
  ...
  *offset = 0;
  ...
  if (size < db->pageSize) {
    ...
    if (spot == NULL) {
      ...
      if (spot != NULL) {
        spot->size = 0;
        VkResult result = vmaMapMemory(gRenderer->vma, spot->stageBuffer->mem,
                                       &spot->hostData);
        if (result != VK_SUCCESS) {
          vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                    "Failed to map memory, VMA error %i.", result);
          return;
        }
      } else {
        return;
    ...
    *offset = spot->size;
    ...
    const VkDeviceSize alignment =
        maxTwo(gRenderer->pd->props.limits.minStorageBufferOffsetAlignment,
               gRenderer->pd->props.limits.minUniformBufferOffsetAlignment);
    if (size % alignment != 0) {
...
void vk2dDescriptorBufferEndFrame(VK2DDescriptorBuffer db,
  ...
  // Unmap all of the buffers then queue a buffer copy if their size is greater
  // than 0
  for (int i = 0; i < db->bufferCount; i++) {
    vmaUnmapMemory(gRenderer->vma, db->buffers[i].stageBuffer->mem);
    if (db->buffers[i].size > 0) {
      VkBufferCopy bufferCopy = {0};
      bufferCopy.size = (db->buffers[i].size < db->pageSize)
                            ? db->buffers[i].size
                            : db->pageSize;
      vkCmdCopyBuffer(copyBuffer, db->buffers[i].stageBuffer->buf,
                      db->buffers[i].deviceBuffer->buf, 1, &bufferCopy);
    }
  }
}
...
void vk2dDescriptorBufferRecordCopyPipelineBarrier(VK2DDescriptorBuffer db,
  ...
  for (int i = 0; i < db->bufferCount; i++) {
    if (db->buffers[i].size > 0) {
      ...
      db->memoryBarriers[i].dstAccessMask =
          VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;
      db->memoryBarriers[i].srcQueueFamilyIndex =
          gRenderer->pd->QueueFamily.graphicsFamily;
      db->memoryBarriers[i].dstQueueFamilyIndex =
          gRenderer->pd->QueueFamily.graphicsFamily;
  ...
  vkCmdPipelineBarrier(buf, VK_PIPELINE_STAGE_TRANSFER_BIT,
                       ...
                       VK_NULL_HANDLE, barrierCount, db->memoryBarriers, 0,
                       VK_NULL_HANDLE);
}
...
void vk2dDescriptorBufferRecordComputePipelineBarrier(VK2DDescriptorBuffer db,
  ...
  vkCmdPipelineBarrier(buf, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                       VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, 0, 0, VK_NULL_HANDLE,
                       barrierCount, db->memoryBarriers, 0, VK_NULL_HANDLE);
}
