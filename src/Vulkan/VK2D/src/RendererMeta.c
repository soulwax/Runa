...
vec2 unitSquareOutline[] = {{0, 0}, {1, 0}, {1, 1}, {0, 1}};
uint32_t unitSquareOutlineVertices = 4;

const vec2 LINE_VERTICES[] = {{0, 0}, {1, 0}};
...
void _vk2dCameraUpdateUBO(VK2DUniformBufferObject *ubo, VK2DCameraSpec *camera,
  ...
  if (camera->type == VK2D_CAMERA_TYPE_DEFAULT) {
    // Assemble view
    mat4 view = {0};
    vec3 eyes = {camera->x + (camera->w * 0.5), camera->y + (camera->h * 0.5),
                 -2};
...
void _vk2dRendererDrawRawShadows(VkDescriptorSet set,
                                 ...
                                 VK2DShadowObject object, vec4 colour,
                                 vec2 lightSource, VK2DCameraIndex cam) {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  if (vk2dStatusFatal())
    return;
  VkCommandBuffer buf = gRenderer->commandBuffer[gRenderer->scImageIndex];
...
void _vk2dRendererDrawRaw3D(VkDescriptorSet *sets, uint32_t setCount,
                            ...
                            float zscale, float rot, vec3 axis, float originX,
                            float originY, float originZ, VK2DCameraIndex cam,
                            float lineWidth) {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  if (vk2dStatusFatal())
    return;
...
void _vk2dRendererDraw3D(VkDescriptorSet *sets, uint32_t setCount,
                         ...
                         float rot, vec3 axis, float originX, float originY,
