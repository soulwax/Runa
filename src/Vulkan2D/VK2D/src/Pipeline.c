// File: src/Vulkan2D/VK2D/src/Pipeline.c
...
VK2DPipeline
vk2dPipelineCreate(VK2DLogicalDevice dev, VkRenderPass renderPass,
                   uint32_t width, uint32_t height, unsigned char *vertBuffer,
                   uint32_t vertSize, unsigned char *fragBuffer,
                   uint32_t fragSize, VkDescriptorSetLayout *setLayouts,
                   uint32_t layoutCount,
                   VkPipelineVertexInputStateCreateInfo *vertexInfo, bool fill,
                   VK2DMSAA msaa, VK2DPipelineType type) {
  if (vk2dStatusFatal())
    return NULL;
  ...
  if (pipe != NULL) {
    ...
    VkShaderModuleCreateInfo vertCreateInfo =
        vk2dInitShaderModuleCreateInfo((void *)vertBuffer, vertSize);
    VkShaderModuleCreateInfo fragCreateInfo =
        vk2dInitShaderModuleCreateInfo((void *)fragBuffer, fragSize);
    VkShaderModule vertShader, fragShader;
    VkResult result = vkCreateShaderModule(dev->dev, &vertCreateInfo,
                                           VK_NULL_HANDLE, &vertShader);
    VkResult result2 = vkCreateShaderModule(dev->dev, &fragCreateInfo,
                                            VK_NULL_HANDLE, &fragShader);
    const uint32_t shaderStageCount = 2;
    VkPipelineShaderStageCreateInfo shaderStageCreateInfo[] = {
        vk2dInitPipelineShaderStageCreateInfo(VK_SHADER_STAGE_VERTEX_BIT,
                                              vertShader),
        vk2dInitPipelineShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT,
                                              fragShader),
    };

    if (result != VK_SUCCESS || result2 != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create shader modules, Vulkan error %i/%i.", result,
                result2);
      free(pipe);
      pipe = NULL;
      return NULL;
    ...
    VkPipelineViewportStateCreateInfo pipelineViewportStateCreateInfo =
        vk2dInitPipelineViewportStateCreateInfo(VK_NULL_HANDLE, &scissor);
    VkPipelineRasterizationStateCreateInfo
        pipelineRasterizationStateCreateInfo =
            vk2dInitPipelineRasterizationStateCreateInfo(polygonFill);

    VkPipelineMultisampleStateCreateInfo pipelineMultisampleStateCreateInfo =
        vk2dInitPipelineMultisampleStateCreateInfo((VkSampleCountFlagBits)msaa);
    ...
    VkPipelineDynamicStateCreateInfo pipelineDynamicStateCreateInfo =
        vk2dInitPipelineDynamicStateCreateInfo(states, stateCount);
    ...
    pipelineLayoutCreateInfo =
        vk2dInitPipelineLayoutCreateInfo(setLayouts, layoutCount, 1, &range);
    result = vkCreatePipelineLayout(dev->dev, &pipelineLayoutCreateInfo,
                                    VK_NULL_HANDLE, &pipe->layout);
    VkPipelineInputAssemblyStateCreateInfo
        pipelineInputAssemblyStateCreateInfo =
            vk2dInitPipelineInputAssemblyStateCreateInfo(fill);

    if (result != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create pipeline layout, Vulkan error %i.", result);
      free(pipe);
      pipe = NULL;
    ...
    for (i = 0; i < VK2D_BLEND_MODE_MAX; i++) {
      VkPipelineColorBlendStateCreateInfo pipelineColorBlendStateCreateInfo =
          vk2dInitPipelineColorBlendStateCreateInfo(&VK2D_BLEND_MODES[i], 1);
      VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo =
          vk2dInitGraphicsPipelineCreateInfo(
              shaderStageCreateInfo, shaderStageCount, vertexInfo,
              &pipelineInputAssemblyStateCreateInfo,
              ...
              &pipelineDynamicStateCreateInfo, pipe->layout, renderPass);
      result = vkCreateGraphicsPipelines(dev->dev, VK_NULL_HANDLE, 1,
                                         &graphicsPipelineCreateInfo,
                                         VK_NULL_HANDLE, &pipe->pipes[i]);
      if (result != VK_SUCCESS) {
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to create pipeline, Vulkan error %i.", result);
        break;
      }
    }
...
VK2DPipeline vk2dPipelineCreateCompute(VK2DLogicalDevice dev,
                                       uint32_t pushBufferSize,
                                       unsigned char *shaderBuffer,
                                       uint32_t shaderSize,
  ...
  if (pipe != NULL) {
    ...
    VkResult shaderRes = vkCreateShaderModule(dev->dev, &shaderModuleCreateInfo,
                                              VK_NULL_HANDLE, &shader);
    VkResult res = vkCreatePipelineLayout(dev->dev, &pipelineLayoutCreateInfo,
                                          VK_NULL_HANDLE, &pipe->layout);
    ...
    if (res == VK_SUCCESS && shaderRes == VK_SUCCESS) {
      ...
      res = vkCreateComputePipelines(dev->dev, VK_NULL_HANDLE, 1,
      ...
      if (res != VK_SUCCESS) {
        ...
        pipe = NULL;
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to create compute pipeline, Vulkan error %i.", res);
      }
      ...
      if (res != VK_SUCCESS)
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to create compute pipeline layout, Vulkan error %i.",
                  res);
      if (shaderRes != VK_SUCCESS)
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to create compute shader module, Vulkan error %i.",
                  res);
    }
  } else {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM, "Failed to allocate pipeline struct.");
