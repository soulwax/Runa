...
void _vk2dShaderBuildPipe(VK2DShader shader) {
  ...
  VkDescriptorSetLayout layout[] = {renderer->dslBufferVP, renderer->dslSampler,
                                    renderer->dslTextureArray,
                                    renderer->dslBufferUser};
  ...
  shader->pipe = vk2dPipelineCreate(
      renderer->ld, renderer->renderPass, renderer->surfaceWidth,
      renderer->surfaceHeight, shader->spvVert, shader->spvVertSize,
      shader->spvFrag, shader->spvFragSize, layout, layoutCount,
      &textureVertexInfo, true, renderer->config.msaa,
...
VK2DShader vk2dShaderFrom(const uint8_t *vertexShaderBuffer,
  ...
  if (uniformBufferSize % 4 != 0) {
    vk2dRaise(
        ...
        "Uniform buffer size for shader is invalid, must be multiple of 4");
    return NULL;
  } else if (uniformBufferSize > gRenderer->limits.maxShaderBufferSize) {
    vk2dRaise(VK2D_STATUS_BEYOND_LIMIT,
              "Uniform buffer of size %i is greater than the maximum allowed "
              "uniform buffer size of %i from vk2dRendererGetLimits",
  ...
  uint8_t *vertFile =
      _vk2dCopyBuffer(vertexShaderBuffer, vertexShaderBufferSize);
  if (vertFile == NULL)
    return NULL;

  uint8_t *fragFile =
      _vk2dCopyBuffer(fragmentShaderBuffer, fragmentShaderBufferSize);
  if (vertFile == NULL) {
    free(vertFile);
    return NULL;
  }
  ...
  if (out != NULL) {
    ...
    if (!gRenderer->limits.supportsMultiThreadLoading ||
        SDL_TryLockMutex(dev->shaderMutex)) {
      _vk2dRendererAddShader(out);
      _vk2dShaderBuildPipe(out);
      if (gRenderer->limits.supportsMultiThreadLoading)
        SDL_UnlockMutex(dev->shaderMutex);
    } else {
      vk2dRaise(VK2D_STATUS_SDL_ERROR, "Failed to lock mutex, SDL error: %s.",
                SDL_GetError());
      free(out);
      out = NULL;
    }
...
VK2DShader vk2dShaderLoad(const char *vertexShader, const char *fragmentShader,
                          uint32_t uniformBufferSize) {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  if (vk2dStatusFatal() || gRenderer == NULL)
    return NULL;
