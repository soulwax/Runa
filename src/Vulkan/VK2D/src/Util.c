// File: src/Vulkan2D/VK2D/src/Util.c
  ...
  if (!init) {
    ...
    vertexInputAttributeDescription[4] =
        vk2dInitVertexInputAttributeDescription(
            ...
            offsetof(VK2DDrawInstance, model) + (sizeof(vec4) * 1));
    vertexInputAttributeDescription[5] =
        vk2dInitVertexInputAttributeDescription(
            5, 0, VK_FORMAT_R32G32B32A32_SFLOAT,
            offsetof(VK2DDrawInstance, model) + (sizeof(vec4) * 2));
    vertexInputAttributeDescription[6] =
        vk2dInitVertexInputAttributeDescription(
            6, 0, VK_FORMAT_R32G32B32A32_SFLOAT,
            offsetof(VK2DDrawInstance, model) + (sizeof(vec4) * 3));
    pipelineVertexInputStateCreateInfo =
        vk2dInitPipelineVertexInputStateCreateInfo(
            &vertexInputBindingDescription, 1, vertexInputAttributeDescription,
            7);
...
VkPipelineVertexInputStateCreateInfo _vk2dGetShadowsVertexInputState() {
  ...
  if (!init) {
    vertexInputBindingDescription = vk2dInitVertexInputBindingDescription(
        VK_VERTEX_INPUT_RATE_VERTEX, sizeof(vec3), 0);
    vertexInputAttributeDescription[0] =
...
int _vk2dWorkerThread(void *data) {
  ...
  VkCommandPoolCreateInfo commandPoolCreateInfo2 =
      vk2dInitCommandPoolCreateInfo(
          dev->pd->QueueFamily.graphicsFamily,
          VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
  VkResult result = vkCreateCommandPool(dev->dev, &commandPoolCreateInfo2,
                                        VK_NULL_HANDLE, &dev->loadPool);
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to create command buffer for worker thread.");
  }
    ...
    if (SDL_GetAtomicInt(&dev->loads) > 0) {
      ...
      for (int i = 0; i < SDL_GetAtomicInt(&dev->loadListSize) && !found; i++) {
        if (dev->loadList[i].state == VK2D_ASSET_TYPE_ASSET) {
          ...
          if (dev->loadList[i].type != VK2D_ASSET_TYPE_MODEL_FILE &&
              dev->loadList[i].type != VK2D_ASSET_TYPE_MODEL_MEMORY)
            found = true;
        }
      }
      ...
      if (asset.type == VK2D_ASSET_TYPE_TEXTURE_FILE) {
        ...
        *asset.Output.texture =
            _vk2dTextureFromInternal(asset.Load.data, asset.Load.size, false);
        if (*asset.Output.texture == NULL)
          vk2dLogInfo("Failed to load texture from buffer.");
      } else if (asset.type == VK2D_ASSET_TYPE_MODEL_FILE) {
        uint32_t size;
        uint8_t *fileData = _vk2dLoadFile(asset.Load.filename, &size);
        *asset.Output.model = _vk2dModelFromInternal(
            fileData, size, *asset.Data.Model.tex, false);
        ...
        *asset.Output.model = _vk2dModelFromInternal(
            asset.Load.data, asset.Load.size, *asset.Data.Model.tex, false);
        if (*asset.Output.model == NULL)
        ...
        *asset.Output.shader =
            vk2dShaderLoad(asset.Load.filename, asset.Load.fragmentFilename,
                           asset.Data.Shader.uniformBufferSize);
      } else if (asset.type == VK2D_ASSET_TYPE_SHADER_MEMORY) {
        // Shaders are internally synchronized
        *asset.Output.shader = vk2dShaderFrom(
            asset.Load.data, asset.Load.size, asset.Load.fragmentData,
            asset.Load.fragmentSize, asset.Data.Shader.uniformBufferSize);
      }
...
void vk2dAssetsLoad(VK2DAssetLoad *assets, uint32_t count) {
  ...
  if (gRenderer->limits.supportsMultiThreadLoading) {
    ...
    if (dev->loadList == NULL) {
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                "Failed to create load list for worker thread.");
    }
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (asset.type == VK2D_ASSET_TYPE_TEXTURE_FILE) {
        ...
        *asset.Output.texture =
            _vk2dTextureFromInternal(asset.Load.data, asset.Load.size, true);
        if (*asset.Output.texture == NULL)
          vk2dLogInfo("Failed to load texture from buffer.");
      } else if (asset.type == VK2D_ASSET_TYPE_MODEL_FILE) {
        uint32_t size;
        uint8_t *fileData = _vk2dLoadFile(asset.Load.filename, &size);
        *asset.Output.model =
            _vk2dModelFromInternal(fileData, size, *asset.Data.Model.tex, true);
        if (*asset.Output.model == NULL)
          vk2dLogInfo("Failed to load model \"%s\".", asset.Load.filename);
        free(fileData);
      } else if (asset.type == VK2D_ASSET_TYPE_MODEL_MEMORY) {
        *asset.Output.model = _vk2dModelFromInternal(
            asset.Load.data, asset.Load.size, *asset.Data.Model.tex, true);
        if (*asset.Output.model == NULL)
          vk2dLogInfo("Failed to load model from buffer.");
      } else if (asset.type == VK2D_ASSET_TYPE_SHADER_FILE) {
        // Shaders are internally synchronized
...
void vk2dAssetsFree(VK2DAssetLoad *assets, uint32_t count) {
  vk2dRendererWait();
  for (int i = 0; i < count; i++) {
    if (assets[i].type == VK2D_ASSET_TYPE_TEXTURE_FILE ||
        assets[i].type == VK2D_ASSET_TYPE_TEXTURE_MEMORY)
      vk2dTextureFree(*assets[i].Output.texture);
    else if (assets[i].type == VK2D_ASSET_TYPE_SHADER_FILE ||
             assets[i].type == VK2D_ASSET_TYPE_SHADER_MEMORY)
      vk2dShaderFree(*assets[i].Output.shader);
    else if (assets[i].type == VK2D_ASSET_TYPE_MODEL_FILE ||
             assets[i].type == VK2D_ASSET_TYPE_MODEL_MEMORY)
