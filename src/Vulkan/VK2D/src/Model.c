...
VK2DModel _vk2dModelCreateInternal(const VK2DVertex3D *vertices,
                                   uint32_t vertexCount,
                                   const uint16_t *indices, uint32_t indexCount,
                                   VK2DTexture tex, bool mainThread) {
  VK2DModel model = malloc(sizeof(struct VK2DModel_t));
  if (model != NULL) {
    VK2DBuffer buf = vk2dBufferLoad2(
        vk2dRendererGetDevice(), sizeof(VK2DVertex3D) * vertexCount,
        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
        (void *)vertices, sizeof(uint16_t) * indexCount, (void *)indices,
        mainThread);
...
VK2DModel vk2dModelCreate(const VK2DVertex3D *vertices, uint32_t vertexCount,
                          const uint16_t *indices, uint32_t indexCount,
                          VK2DTexture tex) {
  VK2DModel model = _vk2dModelCreateInternal(vertices, vertexCount, indices,
                                             indexCount, tex, true);
  return model;
}

static inline bool verticesAreEqual(VK2DVertex3D *v1, VK2DVertex3D *v2) {
  if (v1->uv[0] == v2->uv[0] && v1->uv[1] == v2->uv[1] &&
...
VK2DModel _vk2dModelFromInternal(const void *objFile, uint32_t objFileSize,
  ...
  int status = tinyobj_parse_obj(&attrib, &shapes, &num_shapes, &materials,
                                 &num_materials, "abcdef", _getFileData, NULL,
                                 TINYOBJ_FLAG_TRIANGULATE);

  if (status == TINYOBJ_SUCCESS) {
    // Create lists for the worst case, actual parsed indices/vertices will
    // likely we less
    VK2DVertex3D *vertices =
        malloc(sizeof(VK2DVertex3D) * attrib.num_faces * 3);
    ...
    if (vertices != NULL && indices != NULL) {
      for (int faceIndex = 0; faceIndex < attrib.num_faces; faceIndex++) {
        ...
        // Check if this vertex is the same as any of the previous 6 (kinda
        // arbitrary)
        int found = -1;
      ...
      m = _vk2dModelCreateInternal(vertices, vertexCount, indices, indexCount,
                                   texture, mainThread);
    } else {
      m = NULL;
    }
...
VK2DModel vk2dModelFrom(const void *objFile, uint32_t objFileSize,
                        VK2DTexture texture) {
  if (vk2dStatusFatal())
    return NULL;
