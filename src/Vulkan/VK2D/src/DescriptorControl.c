// File: src/Vulkan2D/VK2D/src/DescriptorControl.c
...
// Places another descriptor pool at the end of a given desc con's list,
// extending the list if need be
static void _vk2dDescConAppendList(VK2DDescCon descCon) {
  if (vk2dStatusFatal())
    return;
  if (descCon->poolListSize == descCon->poolsInUse) {
    VkDescriptorPool *newList =
        realloc(descCon->pools,
    ...
    if (newList != NULL) {
      ...
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                "Failed to append descriptor control list.");
      return;
    }
  }
  ...
  VkDescriptorPoolCreateInfo createInfo = vk2dInitDescriptorPoolCreateInfo(
      sizes, i, VK2D_DEFAULT_DESCRIPTOR_POOL_ALLOCATION);
  VkResult result =
      vkCreateDescriptorPool(descCon->dev->dev, &createInfo, VK_NULL_HANDLE,
                             &descCon->pools[descCon->poolsInUse]);
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to create descriptor pool, Vulkan error %i.", result);
  }
  descCon->poolsInUse++;
}

// Gets the first available descriptor set from a descriptor controller
// (allocating a new pool if need be)
VkDescriptorSet _vk2dDescConGetAvailableSet(VK2DDescCon descCon) {
  ...
  VkDescriptorSetAllocateInfo allocInfo =
      vk2dInitDescriptorSetAllocateInfo(VK_NULL_HANDLE, 1, &descCon->layout);
  ...
  while (set == VK_NULL_HANDLE) {
    ...
    if (res == VK_ERROR_OUT_OF_POOL_MEMORY) {
      ...
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create descriptor set, Vulkan error %i.", res);
      break;
    }
    i++;
...
VK2DDescCon vk2dDescConCreate(VK2DLogicalDevice dev,
                              VkDescriptorSetLayout layout, uint32_t buffer,
                              uint32_t sampler, uint32_t storageBuffer) {
  if (vk2dStatusFatal())
    return NULL;
  ...
  if (out != NULL) {
    ...
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to allocate descriptor controller.");
  }
...
void vk2dDescConFree(VK2DDescCon descCon) {
  if (descCon != NULL) {
    ...
    for (i = 0; i < descCon->poolsInUse; i++)
      vkDestroyDescriptorPool(descCon->dev->dev, descCon->pools[i],
                              VK_NULL_HANDLE);
    free(descCon->pools);
    free(descCon);
  }
...
VkDescriptorSet vk2dDescConGetBufferSet(VK2DDescCon descCon,
  ...
  VkWriteDescriptorSet write = vk2dInitWriteDescriptorSet(
      VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, descCon->buffer, set, &bufferInfo, 1,
      VK_NULL_HANDLE);
...
VkDescriptorSet vk2dDescConGetSamplerSet(VK2DDescCon descCon, VK2DTexture tex) {
  ...
  VkWriteDescriptorSet write = vk2dInitWriteDescriptorSet(
      VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, descCon->sampler, set, VK_NULL_HANDLE,
      1, &imageInfo);
  vkUpdateDescriptorSets(descCon->dev->dev, 1, &write, 0, VK_NULL_HANDLE);
  return set;
}
...
VkDescriptorSet vk2dDescConGetSamplerBufferSet(VK2DDescCon descCon,
  ...
  imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
  imageInfo.imageView = tex->img->view;
  write[1] = vk2dInitWriteDescriptorSet(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
                                        descCon->sampler, set, VK_NULL_HANDLE,
                                        1, &imageInfo);
  VkDescriptorBufferInfo bufferInfo = {0};
  bufferInfo.buffer = buffer->buf;
  bufferInfo.offset = 0;
  bufferInfo.range = buffer->size;
  write[0] = vk2dInitWriteDescriptorSet(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                        descCon->buffer, set, &bufferInfo, 1,
                                        VK_NULL_HANDLE);
