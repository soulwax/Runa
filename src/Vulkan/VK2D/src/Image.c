// File: src/Vulkan2D/VK2D/src/Image.c
...
static void _vk2dImageCopyBufferToImage(VK2DLogicalDevice dev, VkBuffer buffer,
                                        VkImage image, uint32_t width,
                                        uint32_t height, bool mainThread) {
  VkCommandBuffer commandBuffer =
      vk2dLogicalDeviceGetSingleUseBuffer(dev, mainThread);
...
VK2DImage vk2dImageCreate(VK2DLogicalDevice dev, uint32_t width,
  ...
  if (out != NULL) {
    ...
    VkImageCreateInfo imageCreateInfo =
        vk2dInitImageCreateInfo(width, height, format, usage, 1, samples);
    VmaAllocationCreateInfo allocationCreateInfo = {0};
    allocationCreateInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
    VkResult result =
        vmaCreateImage(gRenderer->vma, &imageCreateInfo, &allocationCreateInfo,
                       &out->img, &out->mem, VK_NULL_HANDLE);
    if (result == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
      vk2dRaise(VK2D_STATUS_OUT_OF_VRAM,
                "Failed to create image of size %ix%i, out of video memory.",
                width, height);
      free(out);
      out = NULL;
    } else if (result != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create image of size %ix%i, Vulkan error %i.", width,
                height, result);
      free(out);
      out = NULL;
    } else {
      ...
      VkImageViewCreateInfo imageViewCreateInfo =
          vk2dInitImageViewCreateInfo(out->img, format, aspectMask, 1);
      result =
          vkCreateImageView(dev->dev, &imageViewCreateInfo, NULL, &out->view);
      if (result != VK_SUCCESS) {
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to create image view for image of size %ix%i, Vulkan "
                  "error %i.",
                  width, height, result);
...
VK2DImage vk2dImageLoad(VK2DLogicalDevice dev, const char *filename) {
  ...
  if (pixels != NULL) {
    ...
    if (stage != NULL) {
      ...
      out = vk2dImageCreate(
          dev, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB,
          VK_IMAGE_ASPECT_COLOR_BIT,
          VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 1);

      if (out != NULL) {
        _vk2dImageTransitionImageLayout(
            dev, out->img, VK_IMAGE_LAYOUT_UNDEFINED,
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, true);
        _vk2dImageCopyBufferToImage(dev, stage->buf, out->img, texWidth,
                                    texHeight, true);
        _vk2dImageTransitionImageLayout(
            dev, out->img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    vk2dRaise(VK2D_STATUS_FILE_NOT_FOUND, "Failed to load image \"%s\".",
              filename);
  }
  ...
  if (pixels != NULL) {
    ...
    if (stage != NULL) {
      ...
      if (result == VK_SUCCESS) {
        ...
        out = vk2dImageCreate(
            dev, w, h, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT,
            VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, 1);
