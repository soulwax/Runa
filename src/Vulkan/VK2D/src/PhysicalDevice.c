...
_vk2dPhysicalDeviceGetPhysicalDevices(VkInstance instance, uint32_t *size) {
  VkPhysicalDevice *devices;

  // Find out how many devices we have, allocate that amount, then fill in the
  // blanks
    ...
    if (devices != NULL) {
      ...
      if (result != VK_SUCCESS) {
        ...
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to enumerate devices, Vulkan error %i.", result);
      }
    } else {
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM, "Failed to allocate %i devices.",
                *size);
    }
  } else {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
...
static bool _vk2dPhysicalDeviceSupportsQueueFamilies(VkInstance instance,
                                                     VkPhysicalDevice dev,
                                                     VK2DPhysicalDevice out) {
  ...
  if (queueList != NULL) {
    ...
    for (i = 0; i < queueFamilyCount && !gfx; i++) {
      if (queueList[i].queueCount > 0) {
        if (queueList[i].queueFlags & VK_QUEUE_GRAPHICS_BIT &&
            queueList[i].queueFlags & VK_QUEUE_COMPUTE_BIT) {
          out->QueueFamily.graphicsFamily = i;
          gRenderer->limits.supportsMultiThreadLoading =
              queueList[i].queueCount >= 2;
          gfx = true;
    ...
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to allocate queue family properties.");
  }
...
static bool _vk2dPhysicalDeviceSupported(VkInstance instance,
                                         VkPhysicalDevice dev,
                                         VkPhysicalDeviceProperties *props,
                                         VK2DPhysicalDevice out) {
  bool supportsQueueFamily =
      _vk2dPhysicalDeviceSupportsQueueFamilies(instance, dev, out);
  return supportsQueueFamily;
}

VkPhysicalDeviceProperties *vk2dPhysicalDeviceGetList(VkInstance instance,
                                                      uint32_t *size) {
  VkPhysicalDevice *devs =
      _vk2dPhysicalDeviceGetPhysicalDevices(instance, size);
  ...
  if (devs != NULL) {
    ...
    if (props != NULL) {
      ...
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                "Failed to allocate device properties.");
    }
  }
...
static VkPhysicalDevice
_vk2dPhysicalDeviceGetBestDevice(VkInstance instance, VK2DPhysicalDevice out,
                                 int32_t preferredDevice, bool *foundPrimary) {
  uint32_t devCount;
  VkPhysicalDevice choice = VK_NULL_HANDLE;
  VkPhysicalDeviceProperties choiceProps;
  *foundPrimary = false;
  VkPhysicalDevice *devs =
      _vk2dPhysicalDeviceGetPhysicalDevices(instance, &devCount);
  ...
  if (preferredDevice != VK2D_DEVICE_BEST_FIT) {
    if (preferredDevice < devCount && devCount >= 0) {
      ...
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR, "Device \"%i\" out of range.",
                preferredDevice);
      free(out);
      out = NULL;
    }
...
VK2DPhysicalDevice vk2dPhysicalDeviceFind(VkInstance instance,
  ...
  if (out != NULL) {
    choice = _vk2dPhysicalDeviceGetBestDevice(instance, out, preferredDevice,
                                              &foundPrimary);
    vkGetPhysicalDeviceProperties(choice, &choiceProps);

    // Check if we found one and print if it was discrete (dedicated) or
    // otherwise
    if (choice != VK_NULL_HANDLE) {
      vk2dLogInfo(foundPrimary ? "Found discrete device %s [Vulkan %i.%i.%i]"
                  ...
                  VK_VERSION_MAJOR(choiceProps.apiVersion),
                  VK_VERSION_MINOR(choiceProps.apiVersion),
                  VK_VERSION_PATCH(choiceProps.apiVersion));
...
VK2DMSAA vk2dPhysicalDeviceGetMSAA(VK2DPhysicalDevice physicalDevice) {
  VkSampleCountFlags counts =
      physicalDevice->props.limits.framebufferColorSampleCounts &
      physicalDevice->props.limits.framebufferDepthSampleCounts;
...
void vk2dPhysicalDeviceFree(VK2DPhysicalDevice dev) { free(dev); }
