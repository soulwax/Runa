/**
 * main_rpg.cpp
 * The Slime Hunter - A top-down action RPG
 */
...
class SlimeHunterRPG : public Runa::Application {
public:
	SlimeHunterRPG() : Application("The Slime Hunter - RPG Demo", 1280, 720) {}

protected:
	void onInit() override {
		LOG_INFO("=== The Slime Hunter RPG ===");
		srand(static_cast<unsigned>(time(nullptr)));

		// Initialize systems
		m_registry = std::make_unique<Runa::ECS::EntityRegistry>();
		m_spriteBatch = std::make_unique<Runa::SpriteBatch>(getRenderer());
		m_font = std::make_unique<Runa::Font>(getRenderer(), "Resources/Fonts/Renogare.ttf", 20);
	...
	void createQuestGiver() {
		...
		questGiver.quest.description = "The village is under attack! Defeat 5 slimes.";
			...
			if (playerView.size_hint() != 0) {
				...
				for (auto npc : questView) {
					...
					if (dist < 80.0f) {
						...
						} else if (questGiver.quest.status == Runa::ECS::QuestStatus::Completed) {
	...
	void onRender() override {
		...
		Runa::ECS::RPGSystems::renderDamageNumbers(m_registry->getRegistry(), *m_spriteBatch, *m_font, *m_camera);

		// Render UI
		Runa::ECS::RPGSystems::renderPlayerUI(m_registry->getRegistry(), *m_spriteBatch, *m_font, 1280, 720);

		// Render quest text
		if (m_showQuestText) {
			auto texture = m_font->renderText("New Quest: Slime Infestation!", {255, 255, 100, 255});
			if (texture) {
	...
	void renderInventory() {
		...
		if (inv.items.empty()) {
			...
			for (const auto& item : inv.items) {
				std::string itemText = item.name + " x" + std::to_string(item.stackSize);
...
int main(int argc, char* argv[]) {
	Runa::Log::init();
	LOG_INFO("Starting The Slime Hunter RPG");
